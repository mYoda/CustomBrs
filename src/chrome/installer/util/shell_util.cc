...
class RegistryEntry {
 public:
  ...
  static void GetShellIntegrationEntries(BrowserDistribution* dist,
    ...
    // Register for the Start Menu "Alfa" link (pre-Win7).
    ...
    // Register the "open" verb for launching Chrome via the "Alfa" link.
    entries->push_back(new RegistryEntry(
        start_menu_entry + ShellUtil::kRegShellOpen, quoted_exe_path));
    // Register Chrome's icon for the Start Menu "Alfa" link.
    entries->push_back(new RegistryEntry(
