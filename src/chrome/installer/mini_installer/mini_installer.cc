...
BOOL CALLBACK OnResourceFound(HMODULE module, const wchar_t* type,
  ...
  if (StrStartsWith(name, kChromePrefix)) {
    ...
    // anything else.
	  //::MessageBox(0, L"Some", name, MB_ICONERROR);
    return FALSE;
  }
...
bool UnpackBinaryResources(const Configuration& configuration, HMODULE module,
  ...
  if (!setup_dest_path.assign(base_path) ||
	  ...
	  //::MessageBox(0, L"1", 0, MB_ICONERROR);
	  return false;
  }
  ...
  if (!::EnumResourceNames(module, kLZMAResourceType, OnResourceFound,
	  ...
	  //::MessageBox(0, archive_path->get(), 0, MB_ICONERROR);
	  return false;
  }
  ...
  if (setup_path->length() > 0) {
    ...
    if (!GetSetupExePathFromRegistry(configuration, cmd_line.get(),
        ...
        !cmd_line.append(L"\"")) {
		//::MessageBox(0, L"3", 0, MB_ICONERROR);
      success = false;
    }
    ...
    if (success &&
		...
		//::MessageBox(0, L"4", 0, MB_ICONERROR);
      success = false;
    }
	...
	if (!success)
	{
		...
		//::MessageBox(0, L"DeleteFile", 0, MB_ICONERROR);
	}
...
int WMain(HMODULE module) {
  ...
  if (!configuration.Initialize()){
	  //::MessageBox(NULL, L"Parse the command line.", NULL, MB_ICONERROR);
	  return exit_code;
  }
  ...
  if (ProcessNonInstallOperations(configuration, &exit_code)){
	  //::MessageBox(NULL, L"--cleanup .", NULL, MB_ICONERROR);
	  return exit_code;
  }
  ...
  if (!GetWorkDir(module, &base_path)){
	  //::MessageBox(NULL, base_path.get(), L"base_path", MB_ICONERROR);
	  return 101;
  }
  ...
  if (!UnpackBinaryResources(configuration, module, base_path.get(),
	  ...
	  //::MessageBox(NULL, base_path.get(), NULL, MB_ICONERROR);
    exit_code = 102;
  } else {
  ...
  if (ShouldDeleteExtractedFiles()){
	  ...
	  //::MessageBox(NULL, L"DeleteExtractedFiles", NULL, MB_ICONERROR);
  }

  //::MessageBox(NULL, L"exit_code", NULL, MB_ICONERROR);
  return exit_code;
}
